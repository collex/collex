  ...
  def reset_password
		if request.post? and params[:help_username] and params[:help_username].size > 0
          ...
          body += "We strongly recommend that you change your password by going to the #{Setup.site_name()} site and clicking the #{Setup.my_collex()} tab. Then select EDIT PROFILE."
					...
					EmailWaiting.cue_email(Setup.site_name(), Setup.return_email(), @user[:fullname], @user[:email], "Password Reset", body, url_for(:controller => 'home', :action => 'index', :only_path => false), "")
		...
		if request.post? and params[:help_email] and params[:help_email].size > 0
			...
			if @user != nil
				begin
					...
					EmailWaiting.cue_email(Setup.site_name(), Setup.return_email(), @user[:fullname], @user[:email], "Recover User Name", body, url_for(:controller => 'home', :action => 'index', :only_path => false), "")
