...
class CachedResource < ActiveRecord::Base
  ...
  CLOUD_SQL = { 
    :archive => "select value as name, count(value) as freq from cached_properties as props join cached_resources as docs on docs.id=props.cached_resource_id join interpretations as i on docs.uri=i.object_uri  where props.name = 'archive'  group by value order by freq,name limit ?",
    :agent_facet => "select value as name, count(value) as freq from cached_properties as agents join cached_resources as docs on docs.id=agents.cached_resource_id join interpretations as i on docs.uri=i.object_uri where agents.name like 'role_%' group by value order by freq,value limit ?", 
    ...
    :genre => "select value as name, count(value) as freq from cached_properties as genres join cached_resources as docs on docs.id=genres.cached_resource_id join interpretations as i on docs.uri=i.object_uri  where genres.name = 'genre'  group by value order by freq,name limit ?",     
    ...
    :year => "select value as name, count(value) as freq from cached_properties as dates join cached_resources as docs on dates.cached_resource_id=docs.id where dates.name = 'date_label' group by dates.value order by freq,value limit ?"
  }
  
  CLOUD_BY_USER_SQL = { 
    :archive => "select value as name, count(value) as freq from cached_properties as props join cached_resources as docs on docs.id=props.cached_resource_id join interpretations as i on docs.uri=i.object_uri  where user_id=? and props.name = 'archive' group by value order by freq,value limit ?",
    :agent_facet => "select value as name, count(value) as freq from cached_properties as agents join cached_resources as docs on docs.id=agents.cached_resource_id join interpretations as i on docs.uri=i.object_uri where user_id=? and agents.name like 'role_%' group by value order by freq,value limit ?", 
    ...
    :genre => "select value as name, count(value) as freq from cached_properties as genres join cached_resources as docs on docs.id=genres.cached_resource_id join interpretations as i on docs.uri=i.object_uri  where user_id=? and genres.name = 'genre' group by value order by freq,value limit ?",
    ...
    :year => "select value as name, count(value) as freq from cached_properties as dates join cached_resources as docs on dates.cached_resource_id=docs.id join interpretations as i on docs.uri=i.object_uri where user_id=? and dates.name = 'date_label' group by dates.value order by freq,value limit ?"      
  }
