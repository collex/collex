class EmailWaiting < ActiveRecord::Base
	def self.periodic_update
		...
		emails.each{|email|
			LoginMailer.deliver_generic(email.from_name, email.from_email, email.to_name, email.to_email, email.subject, email.body, email.return_url)
			email.destroy
		}
		return { :activity => true, :message => "Number of emails sent: #{emails.length}" }
	end

	def self.cue_email(from_name, from_email, to_name, to_email, subject, body, return_url)
		EmailWaiting.create({ :from_name => from_name, :from_email => from_email, :to_name => to_name,
			:to_email => to_email, :subject => subject, :body => body, :return_url => return_url })
		puts "---------------------"
		puts "cue_email:"
