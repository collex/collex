...
class Solr
  ...
  def search(constraints, start, max)
    ...
    post_data = "fl=title,genre,year,date_label,archive,agent,uri,url,archive,thumbnail,source&start=#{start}&rows=#{max}&qt=facet&ff=genre&ff=archive&wt=ruby&highlight=on&highlightFields=text"
    post_data << encode_constraints(constraints)
