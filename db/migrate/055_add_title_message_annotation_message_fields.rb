class AddTitleMessageAnnotationMessageFields < ActiveRecord::Migration
  ...
  def self.up
    ...
    ExhibitType.update ann_bib.id, :title_message => "(Insert Bibliography Title)", :annotation_message => "(Insert General Description)"

    ill_ess = ExhibitType.find_by_template "illustrated_essay"
    ExhibitType.update ill_ess.id, :title_message => "(Insert Essay Title)", :annotation_message => "(Insert Abstract)"
    
    ann_bib_page = ExhibitPageType.find_by_name "Annotated Bibliography Page Type"
    ExhibitPageType.update ann_bib_page.id, :title_message => "(Insert Page Title)", :annotation_message => "(Insert Page Notes)"

    ill_ess_page = ExhibitPageType.find_by_name "Illustrated Essay Page Type"
    ExhibitPageType.update ill_ess_page.id, :title_message => "(Insert Page Title)", :annotation_message => "(Insert Page Notes)"
    
    ann_bib_section = ExhibitSectionType.find_by_template "citation"
    ExhibitSectionType.update ann_bib_section.id, :title_message => "(Insert Section Title)", :annotation_message => "(Insert Section Description)"

    ill_ess_section = ExhibitSectionType.find_by_template "ie_generic"
    ExhibitSectionType.update ill_ess_section.id, :title_message => "(Insert Section Title)", :annotation_message => "(Insert Text)"
  end
