...
var ResultRowDlg = Class.create({
	initialize: function (populate_action, uri, progress_img) {
		// This puts up a modal dialog that allows the administrator to change information about a site or category.
		...
		// private functions
		var populate = function() {
			new Ajax.Request(populate_action, { method: 'get', parameters: { uri: uri },
				onSuccess : function(resp) {
					dlg.setFlash('', false);
					try {
					...
					var hidden_els = details.select(".hidden");
