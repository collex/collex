...
/*extern serverAction, serverNotify, serverRequest, postLink, formatFailureMsg, submitForm, reloadPage, gotoPage, openInNewWindow, submitFormWithConfirmation, ConfirmDlg */
...
var postLink = function(link, params, target) {
	if (window.mockSubmit) {
		...
		f.appendChild(new Element('input', { id: csrf_param, type: 'hidden', name: csrf_param, value: csrf_token }));
		...
		if (params) {
			$H(params).each(function (p) {
				...
				if (typeof p.value === 'string')
					f.appendChild(new Element('input', {type: 'hidden', name: p.key, value: p.value, id: p.key}));
				else if (typeof p.value === 'number') {
					f.appendChild(new Element('input', {type: 'hidden', name: p.key, value: "" + p.value, id: p.key}));
				} else {
					$H(p.value).each(function (pp) {
						f.appendChild(new Element('input', {type: 'hidden', name: p.key + '[' + pp.key + ']', value: pp.value, id: pp.key}));
					});
				}
			});
		}
...
function submitForm(id, action, method) {
	...
	var form = $(id);
	if (method === 'PUT')
		form.appendChild(new Element('input', { id: '_method', type: 'hidden', name: '_method', value: "PUT" }));
	if (method === undefined || method === 'PUT')
		method = 'POST';
	form.writeAttribute({ action: action, method: method });
	...
	initialize: function (title, message, okStr, cancelStr, action) {
		...
		var dlgLayout = {
				...
				rows: [
					...
					[ {rowClass: 'gd_last_row'}, {button: okStr, callback: this.ok, isDefault: true}, {button: cancelStr, callback: GeneralDialog.cancelCallback} ]
				]
			};

		var dlgParams = {this_id: "gd_confirm_dlg", pages: [ dlgLayout ], body_style: "gd_confirm_dlg", row_style: "gd_confirm_row", title: title};
...
var serverAction = function(params) {
	...
	var recurseUpdateWithAjax = function(rParams, resp)
	{
		...
		var updateWithAjax = function(uParams)
		{
			...
			ajaxCall({action: action, params: ajaxParams, el: uParams.el, onSuccess: uParams.onSuccess, dlg: uParams.dlg, onFailure: uParams.onFailure });
		};
		...
		var ajaxparams = {action: action, el: el, onSuccess: function(resp) {recurseUpdateWithAjax({actions: actions, els: els, target: actionParams.target, onSuccess: onSuccess, dlg: rParams.dlg, onFailure: onFailure, params: dlgParams}, resp);}, dlg: rParams.dlg, onFailure: onFailure, params: dlgParams};
...
var serverNotify = function(url, params) {
	serverAction({ action: { actions: url, els: 'gd_bit_bucket', params: params }});
};
...
var serverRequest = function(params) {
	serverAction({ action: { actions: params.url, els: 'gd_bit_bucket', params: params.params, onSuccess: params.onSuccess, onFailure: params.onFailure, dlg: params.dlg }});
};
