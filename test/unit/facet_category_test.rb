...
class FacetCategoriesTest < Test::Unit::TestCase
  ...
  def test_to_facet_tree
    merged = @archive.merge_facets({'rossetti' => 1869})
  ...
  def test_totaling
    ...
    merged = @archive.merge_facets(facets)

    uncategorized = merged.last
    assert_equal("Uncategorized", uncategorized[:value])
    assert_equal(0, uncategorized[:count])
